...
fn pad_eq_(string: &str, length: usize) -> String {
  match string.len() {
...
impl<'a> PadEq for Vec<&'a str> {
  ...
  fn pad_eq(&self) -> Self::Output {
    let len = self.iter().map(|v| v.len()).max().unwrap_or(0);
    let mut result = Vec::new();
    for value in self {
      result.push(pad_eq_(value, len))
    }
  ...
  K: Hash + Eq + Clone,
{
  ...
  fn pad_eq(&self) -> Self::Output {
    let len = self.iter().map(|(_, v)| v.len()).max().unwrap_or(0);
    let mut result = HashMap::new();
    for (key, value) in self {
      result.insert(key.to_owned(), pad_eq_(value, len));
    }
